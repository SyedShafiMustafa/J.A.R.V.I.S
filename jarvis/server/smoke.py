...
server/smoke.py
---------------
Phase 0 test suite for the JARVIS V2 Dell server.  Self-contained — no pytest
required, mirroring the style of backend/smoke.py.

    python server/smoke.py

Sections:
    1. config     — env-driven defaults, overrides, fail-fast validation
    2. logging    — rotating file handler actually writes
    3. db         — sqlite bootstrap: WAL, schema version, reopen
    4. endpoints  — live FastAPI app: /healthz /readyz / /openapi.json + 404s
                    (skipped with a note when fastapi/uvicorn are not installed)
...
def test_endpoints() -> None:
    ...
    if started:
        ...
        data = json.loads(body)
        ok("/readyz -> 200 + database ok", status == 200 and data.get("checks", {}).get("database") == "ok")
        ...
        ready, _ = _http_get(f"{base}/readyz")
